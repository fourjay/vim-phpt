if !exists('g:test#phpt#phpunit#file_pattern')
  let g:test#phpt#phpunit#file_pattern = '.*\.phpt$'
endif

if !exists('g:test#phpt#phpunit#test_patterns')
  ...
  let g:test#phpt#phpunit#test_patterns = {
    \ 'test': [
...
function! test#phpt#phpunit#test_file(file) abort
    return 1
    return a:file =~# g:test#phpt#phpunit#file_pattern
endfunction

function! test#phpt#phpunit#build_position(type, position) abort
    if a:type ==# 'nearest'
        let name = s:nearest_test(a:position)
        if !empty(name) | let name = '--filter '.shellescape('::'.name, 1) | endif
        if !empty(name) && test#phpt#phpunit#executable() =~ 'paratest'
...
function! test#phpt#phpunit#build_args(args, color) abort
    let args = a:args
...
function! s:nearest_test(position) abort
    ...
    let name = test#base#nearest_test(a:position, g:test#phpt#phpunit#test_patterns)
